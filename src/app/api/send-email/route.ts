...
export async function POST(request: NextRequest) {
  try {
    ...
    // Template do email
    const mailOptions = {
      from: `"Formulário Lelume" <${process.env.SMTP_USER}>`,
      to: process.env.EMAIL_TO || 'arthur@lelume.com.br',
      cc: process.env.SMTP_USER, // Cópia para noreplylelume@gmail.com
      replyTo: email,
      subject: `[Lelume] Novo contato - ${assunto.charAt(0).toUpperCase() + assunto.slice(1)}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background-color: #1E7BFF; color: white; padding: 15px; text-align: center; border-radius: 0 0 8px 8px;">
            <p style="margin: 0; font-size: 12px;">Este email foi enviado através do formulário de contato do site Lelume.</p>
          </div>
        </div>
      `,
      text: `
        Nova Mensagem de Contato - Lelume
